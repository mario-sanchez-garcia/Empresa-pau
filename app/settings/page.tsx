...
export default function SettingsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#f9f9f9] text-slate-900 lg:flex">
      <Sidebar activeItem="settings" email={email} />
      <main className="w-full flex-1 px-5 py-8 sm:px-8 lg:px-12">
        <div className="mx-auto max-w-4xl">
          <header className="mb-8">
            <div className="text-xs font-black uppercase tracking-[0.08em] text-[#1c1c1c]">Ajustes / Perfil</div>
          ...
          <div className="grid gap-8">
            <section className="border-y border-[#e8e8e8] bg-white px-5 py-6 sm:px-7">
              <SectionTitle icon={<UserRound size={18} />} title="Perfil" />
              <div className="mt-5 flex flex-wrap items-center gap-5">
                <div className="flex h-24 w-24 items-center justify-center overflow-hidden rounded-full border-2 border-[#e8e8e8] bg-[#f5f5f5] text-2xl font-black text-[#1c1c1c]">
                ...
                <div className="flex gap-2">
                  ...
                  <button type="button" onClick={() => fileRef.current?.click()} className="flex items-center gap-2 rounded-[4px] bg-[#111] px-4 py-2.5 text-xs font-black text-white"><Camera size={15} />Elegir foto</button>
                  {preferences.photo && <button type="button" onClick={() => setPreferences(current => ({ ...current, photo: '' }))} aria-label="Eliminar foto" className="flex h-10 w-10 items-center justify-center rounded-[4px] border border-[#e8e8e8] text-[#1c1c1c]"><X size={16} /></button>}
                </div>
              </div>
              <div className="mt-6 grid gap-4 sm:grid-cols-2">
                <Field label="Nombre visible"><input value={preferences.displayName} onChange={event => setPreferences(current => ({ ...current, displayName: event.target.value }))} placeholder="¿Cómo quieres que te llamemos?" className="w-full rounded-[4px] border border-[#e8e8e8] bg-slate-50 px-3 py-2.5 text-sm outline-none focus:border-[#ccc] focus:bg-white" /></Field>
                <Field label="Email"><input value={email} readOnly className="w-full cursor-not-allowed rounded-[4px] border border-[#e8e8e8] bg-slate-50 px-3 py-2.5 text-sm text-slate-500 outline-none" /></Field>
                <Field label="Comunidad por defecto"><select value={ccaa} onChange={event => setCCAA(event.target.value as CCAA)} className="w-full rounded-[4px] border border-[#e8e8e8] bg-slate-50 px-3 py-2.5 text-sm outline-none focus:border-[#ccc] focus:bg-white">{CCAA_OPTIONS.map(option => <option key={option}>{option}</option>)}</select></Field>
                <Field label="Curso o nivel"><select value={preferences.educationLevel} onChange={event => setPreferences(current => ({ ...current, educationLevel: event.target.value }))} className="w-full rounded-[4px] border border-[#e8e8e8] bg-slate-50 px-3 py-2.5 text-sm outline-none focus:border-[#ccc] focus:bg-white"><option value="1-bachillerato">1.º Bachillerato</option><option value="2-bachillerato">2.º Bachillerato</option><option value="preparacion-pau">Preparación PAU</option><option value="otro">Otro</option></select></Field>
              </div>
            ...
            <section className="border-y border-[#e8e8e8] bg-white px-5 py-6 sm:px-7">
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                <Field label="Estilo de corrección"><select value={preferences.correctionStyle} onChange={event => setPreferences(current => ({ ...current, correctionStyle: event.target.value as Preferences['correctionStyle'] }))} className="w-full rounded-[4px] border border-[#e8e8e8] bg-slate-50 px-3 py-2.5 text-sm outline-none focus:border-[#ccc] focus:bg-white"><option value="breve">Breve</option><option value="normal">Normal</option><option value="detallado">Detallado</option></select></Field>
              </div>
            ...
            <section className="border-y border-[#e8e8e8] bg-white px-5 py-6 sm:px-7">
              ...
              <div className="mt-5 flex flex-wrap items-center justify-between gap-4">
                ...
                <button type="button" onClick={logout} className="flex items-center gap-2 rounded-[4px] border border-red-200 px-4 py-2.5 text-xs font-black text-red-700"><LogOut size={15} />Cerrar sesión</button>
              </div>
            </section>
          </div>
          ...
          <div className="sticky bottom-5 mt-7 flex justify-end">
            <div className="grid justify-items-end gap-2">
              {saveError && <div className="max-w-md rounded-[4px] border border-red-200 bg-red-50 px-4 py-3 text-xs font-semibold text-red-700">{saveError}</div>}
              <button type="button" onClick={save} className="flex items-center gap-2 rounded-[6px] bg-[#111] px-6 py-3.5 text-sm font-black text-white shadow-lg shadow-[rgba(0,0,0,0.08)]"><Save size={17} />{saved ? 'Cambios guardados en este dispositivo' : 'Guardar cambios'}</button>
            </div>
          </div>
        </div>
      </main>
...
function SectionTitle({ icon, title }: { icon: React.ReactNode, title: string }) {
  return <div className="flex items-center gap-2 text-base font-black text-slate-900"><span className="text-[#1c1c1c]">{icon}</span>{title}</div>
}
