...
export async function GET(request: NextRequest) {
  ...
  // Distinct user_ids from pendingRows are already the exact "still has
  // something pending" signal — every id came from a live status='pending'
  // row, so no separate "completed something today" exclusion is needed
  // (see the same fix and longer explanation in daily-reminder/route.ts —
  // that extra check was what silently dropped most weekdays there too).
  const candidates = [...new Set((pendingRows ?? []).map(r => r.user_id as string))]
  if (candidates.length === 0) return NextResponse.json({ sent: 0, skipped: 0, reason: 'no_pending' })
  ...
  // Step 7b: this cron's own candidate logic (pending + not completed today)
  // is a subset of daily-reminder's, which already ran hours earlier the
  // same day — so anyone it already reached doesn't need a second email
  // today too. Without this, an engaged student (streak >= 3) would get
  // both, exactly the "several notifications a day" pattern we want to
  // avoid. Only skip on a confirmed *sent*, so someone whose pending item
  // only appeared after daily-reminder ran still gets reached here.
  const { data: alreadyReminded } = await db
    ...
    .eq('dedupe_key', today)
    .eq('status', 'sent')
    .in('user_id', afterOptOut.map(u => u.userId))
  const alreadyRemindedSet = new Set((alreadyReminded ?? []).map(r => r.user_id as string))
  const afterDedupWithDaily = afterOptOut.filter(u => !alreadyRemindedSet.has(u.userId))
  if (afterDedupWithDaily.length === 0) {
    return NextResponse.json({ sent: 0, skipped: afterOptOut.length, reason: 'already_reminded_today' })
  }
  ...
  const afterDedupWithDailyIds = new Set(afterDedupWithDaily.map(u => u.userId))
  const cutoff = new Date(Date.now() - 2 * 60 * 60 * 1000).toISOString()
  const userMap = new Map(
    [...allUsers.values()].filter(u => afterDedupWithDailyIds.has(u.id) && u.createdAt <= cutoff).map(u => [u.id, u]),
  )
  ...
  await runInBatches(afterDedupWithDaily, 20, async ({ userId, streakDays }) => {
