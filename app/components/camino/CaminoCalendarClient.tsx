...
import { ArrowRight, BarChart3, Check, ChevronDown, Clock3, Plus, RotateCcw, Target, Trash2, Trophy, Zap } from 'lucide-react'
import Sidebar from '@/app/components/Sidebar'
import { loadOnboarding, type OnboardingData } from '@/app/lib/onboarding/onboardingStorage'

type MissionKind = 'exam' | 'flashcards' | 'chat' | 'simulacro' | 'historial'
...
type StudentExam = { id: string; subject: string; date: string; topic: string; name: string; priority: 'normal' | 'alta' }
...
const DIVISIONS = [
  { name: 'Bronce', min: 0, max: 499 }, { name: 'Plata', min: 500, max: 1499 }, { name: 'Oro', min: 1500, max: 3499 },
  { name: 'Platino', min: 3500, max: 6999 }, { name: 'Diamante', min: 7000, max: 12999 }, { name: 'Élite PAU', min: 13000, max: Infinity },
]
const MOCK_RANKING = [
...
function actionHref(kind: MissionKind, subject: string) { const s = subjectSlug(subject); if (kind === 'exam') return `/?subject=${s}&source=camino`; if (kind === 'flashcards') return `/zona?mode=flashcards&subject=${s}&source=camino`; if (kind === 'chat') return `/?view=chat&subject=${s}&source=camino`; if (kind === 'simulacro') return `/simulacros?subject=${s}&source=camino`; return `/?view=historial&subject=${s}&source=camino` }
...
function generateCalendar(onboarding: OnboardingData, exams: StudentExam[]) {
  ...
  return Array.from({ length: 7 }, (_, index): DayPlan => {
    ...
    const upcoming = exams.filter(exam => daysUntil(exam.date) >= 0 && daysUntil(exam.date) <= 2).sort((a, b) => daysUntil(a.date) - daysUntil(b.date))[0]
    const sameDay = exams.find(exam => exam.date === dateISO)
    const studyDay = indexes.includes(index) || Boolean(sameDay)
    const missions: Mission[] = []

    if (studyDay) {
      const prioritySubject = sameDay?.subject ?? (index <= 2 ? upcoming?.subject : null)
      ...
      const reason = sameDay ? `Parcial hoy: ${sameDay.topic || sameDay.name || sameDay.subject}. Repaso ligero.` : upcoming?.subject === subject ? `Parcial cercano: priorizamos ${subject}.` : onboarding.preparationFeeling === 'Me cuesta organizarme' ? 'Poco volumen, mucha claridad.' : 'Reparto equilibrado según tu onboarding.'
...
export default function CaminoCalendarClient() {
  ...
  const [examDraft, setExamDraft] = useState({ subject: '', date: toISO(addDays(new Date(), 3)), topic: '', name: '', priority: 'normal' as 'normal' | 'alta' })
  ...
  function addExam() { if (!examDraft.subject || !examDraft.date) return; const nextExams = [...exams, { id: `${Date.now()}`, ...examDraft }]; setShowExamForm(false); setExamDraft(current => ({ ...current, topic: '', name: '', priority: 'normal' })); regenerate(nextExams) }
  ...
  return (
    <Shell>
      <header className="sticky top-0 z-30 border-b border-blue-100 bg-white/90 px-5 py-4 backdrop-blur-xl"><div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.16em] text-blue-600">Camino PAU</p><h1 className="text-2xl font-black tracking-tight text-slate-950">Tu semana de estudio</h1></div><button onClick={() => setShowExamForm(true)} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-blue-700 to-violet-600 px-5 py-3 text-sm font-black text-white shadow-[0_14px_34px_rgba(37,99,235,0.22)]"><Plus size={16} /> Añadir examen</button></div></header>
      <main className="mx-auto max-w-7xl px-5 py-6">
        <section className="mb-5 grid gap-4 lg:grid-cols-[1.3fr_0.7fr]">
          ...
          <div className="rounded-[28px] border border-blue-100 bg-white p-6 shadow-[0_18px_45px_rgba(37,99,235,0.08)]"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.16em] text-slate-400">XP y división</p><h2 className="mt-2 text-3xl font-black text-slate-950">{totalXP.toLocaleString('es-ES')} XP</h2></div><span className="rounded-2xl bg-violet-50 px-4 py-2 text-sm font-black text-violet-700">{division.name}</span></div><p className="mt-3 text-sm font-semibold text-slate-500">Ganas XP por practicar y aún más cuando mejoras tu precisión.</p><div className="mt-5 h-3 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-gradient-to-r from-blue-700 to-violet-600" style={{ width: `${divisionPct}%` }} /></div><p className="mt-2 text-xs font-bold text-slate-400">{nextDivision ? `Faltan ${Math.max(0, nextDivision.min - totalXP).toLocaleString('es-ES')} XP para ${nextDivision.name}.` : 'División máxima alcanzada.'}</p></div>
        </section>

        <section className="mb-5 rounded-[28px] border border-blue-100 bg-white p-5 shadow-[0_18px_45px_rgba(37,99,235,0.08)]"><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.16em] text-slate-400">Calendario editable</p><h2 className="text-xl font-black text-slate-950">Semana actual</h2></div><p className="text-sm font-bold text-slate-500">{completedMain} de {totalMain} misiones principales completadas</p></div><div className="grid gap-3 lg:grid-cols-7">{calendar.map(day => <DayCard key={day.date} day={day} exams={exams.filter(exam => exam.date === day.date)} onComplete={completeMission} />)}</div></section>

        <section className="grid gap-5 lg:grid-cols-[1fr_0.85fr]"><div className="rounded-[28px] border border-blue-100 bg-white p-5 shadow-[0_18px_45px_rgba(37,99,235,0.08)]"><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-lg font-black text-slate-950">Exámenes parciales</h2><p className="text-sm font-semibold text-slate-500">Añade tus próximos exámenes para que Pausia ajuste tu semana.</p></div><button onClick={() => setShowExamForm(true)} className="inline-flex items-center gap-2 rounded-2xl border border-blue-100 bg-blue-50 px-4 py-2 text-sm font-black text-blue-700"><Plus size={15} /> Añadir examen</button></div><div className="grid gap-2">{exams.length ? exams.map(exam => <div key={exam.id} className="flex items-center justify-between gap-3 rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3"><div className="min-w-0"><p className="truncate text-sm font-black text-slate-800">{exam.subject} · {exam.topic || exam.name || 'Parcial'}</p><p className="text-xs font-bold text-slate-400">{formatDate(exam.date)} · prioridad {exam.priority}</p></div><button onClick={() => deleteExam(exam.id)} className="rounded-xl p-2 text-slate-400 hover:bg-red-50 hover:text-red-600" aria-label="Eliminar examen"><Trash2 size={16} /></button></div>) : <p className="rounded-2xl border border-dashed border-blue-200 bg-blue-50 px-4 py-4 text-sm font-bold text-blue-800">Empieza añadiendo tu próximo examen del instituto.</p>}</div></div><RankingCard open={rankingOpen} setOpen={setRankingOpen} tab={rankingTab} setTab={setRankingTab} rows={visibleRanking} community={onboarding?.community ?? 'Sin comunidad'} totalXP={totalXP} division={division.name} /></section>
      </main>
      <AnimatePresence>{showExamForm && <ExamModal subjects={onboarding?.subjects ?? []} draft={examDraft} setDraft={setExamDraft} onClose={() => setShowExamForm(false)} onSave={addExam} />}</AnimatePresence>
...
function MissionRow({ mission, onComplete, onPostpone, compact = false }: { mission: Mission; onComplete: (id: string) => void; onPostpone: (id: string) => void; compact?: boolean }) {
  ...
  return <div className={`rounded-2xl border p-4 ${mission.status === 'done' ? 'bg-emerald-50 border-emerald-100' : 'bg-white'}`} style={{ borderColor: mission.status === 'done' ? '#bbf7d0' : theme.border }}><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0 flex-1"><div className="mb-2 flex flex-wrap items-center gap-2"><span className="rounded-full px-2.5 py-1 text-[11px] font-black" style={{ background: theme.bg, color: theme.text }}>{mission.subject}</span><span className="inline-flex items-center gap-1 text-[11px] font-bold text-slate-400"><Clock3 size={12} /> {mission.estimatedMinutes} min</span>{mission.role === 'bonus' && <span className="rounded-full bg-violet-50 px-2.5 py-1 text-[11px] font-black text-violet-700">Bonus</span>}</div><h3 className={`${compact ? 'text-sm' : 'text-base'} font-black text-slate-900`}>{mission.title}</h3><p className="mt-1 text-xs font-semibold text-slate-500">{mission.reason}</p></div><div className="flex shrink-0 flex-wrap gap-2"><a href={mission.href} className="inline-flex items-center gap-1.5 rounded-xl bg-blue-600 px-3 py-2 text-xs font-black text-white">Ir <ArrowRight size={13} /></a>{mission.status !== 'done' && <button onClick={() => onComplete(mission.id)} className="inline-flex items-center gap-1.5 rounded-xl border border-emerald-100 bg-emerald-50 px-3 py-2 text-xs font-black text-emerald-700"><Check size={13} /> Hecha</button>}{mission.status !== 'done' && mission.role === 'main' && <button onClick={() => onPostpone(mission.id)} className="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-black text-slate-500"><RotateCcw size={13} /> Posponer</button>}</div></div></div>
}

function DayCard({ day, exams, onComplete }: { day: DayPlan; exams: StudentExam[]; onComplete: (id: string) => void }) {
  const main = day.missions.filter(mission => mission.role === 'main')
  const done = main.length > 0 && main.every(mission => mission.status === 'done')
  return <article className={`min-h-[210px] rounded-3xl border p-3 ${day.isToday ? 'border-blue-300 bg-blue-50/70' : 'border-slate-100 bg-slate-50/80'}`}><div className="mb-3 flex items-center justify-between"><h3 className="text-sm font-black capitalize text-slate-900">{day.label}</h3>{done && <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-black text-emerald-700">Hecho</span>}</div>{exams.map(exam => <p key={exam.id} className="mb-2 rounded-xl bg-amber-50 px-3 py-2 text-[11px] font-black text-amber-800">Parcial: {exam.subject}</p>)}<div className="grid gap-2">{main.length ? main.map(mission => <button key={mission.id} onClick={() => mission.status === 'done' ? undefined : onComplete(mission.id)} className="rounded-2xl border bg-white p-3 text-left transition hover:-translate-y-0.5" style={{ borderColor: themeFor(mission.subject).border }}><p className="text-[11px] font-black" style={{ color: themeFor(mission.subject).text }}>{mission.subject}</p><p className="mt-1 text-xs font-bold text-slate-700">{mission.title}</p><p className="mt-2 text-[11px] font-bold text-slate-400">{mission.status === 'done' ? 'Completada' : 'Pendiente · toca para marcar'}</p></button>) : <p className="text-xs font-semibold text-slate-400">Descanso o repaso libre.</p>}</div></article>
}

function ExamModal({ subjects, draft, setDraft, onClose, onSave }: { subjects: string[]; draft: { subject: string; date: string; topic: string; name: string; priority: 'normal' | 'alta' }; setDraft: (draft: { subject: string; date: string; topic: string; name: string; priority: 'normal' | 'alta' }) => void; onClose: () => void; onSave: () => void }) {
  return <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 grid place-items-center bg-slate-950/30 p-4 backdrop-blur-sm"><motion.div initial={{ scale: 0.96, y: 16 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.96, y: 16 }} className="w-full max-w-lg rounded-[28px] bg-white p-6 shadow-2xl"><h2 className="text-xl font-black text-slate-950">Añadir examen parcial</h2><p className="mt-1 text-sm font-semibold text-slate-500">Si tienes un parcial cerca, lo tendremos en cuenta.</p><div className="mt-5 grid gap-3"><Field label="Asignatura"><select value={draft.subject} onChange={event => setDraft({ ...draft, subject: event.target.value })} className="inputish">{subjects.map(subject => <option key={subject} value={subject}>{subject}</option>)}</select></Field><Field label="Fecha"><input type="date" value={draft.date} onChange={event => setDraft({ ...draft, date: event.target.value })} className="inputish" /></Field><Field label="Tema opcional"><input value={draft.topic} onChange={event => setDraft({ ...draft, topic: event.target.value })} placeholder="Derivadas, Platón, Writing..." className="inputish" /></Field><Field label="Nombre opcional"><input value={draft.name} onChange={event => setDraft({ ...draft, name: event.target.value })} placeholder="Parcial 1" className="inputish" /></Field><Field label="Prioridad"><select value={draft.priority} onChange={event => setDraft({ ...draft, priority: event.target.value as 'normal' | 'alta' })} className="inputish"><option value="normal">Normal</option><option value="alta">Alta</option></select></Field></div><style>{`.inputish{width:100%;border-radius:14px;border:1px solid #dbe7fb;background:#f8fbff;padding:11px 12px;font-size:14px;font-weight:700;color:#334155;outline:none}.inputish:focus{border-color:#93c5fd;background:white}`}</style><div className="mt-6 flex justify-end gap-2"><button onClick={onClose} className="rounded-2xl border border-slate-200 px-4 py-2 text-sm font-black text-slate-500">Cancelar</button><button onClick={onSave} className="rounded-2xl bg-blue-600 px-4 py-2 text-sm font-black text-white">Guardar examen</button></div></motion.div></motion.div>
}
