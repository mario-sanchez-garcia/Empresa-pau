...
export function CanvasToolbar(props: CanvasToolbarProps) {
  ...
  return (
    <>
      <aside className="w-56 shrink-0 overflow-hidden border-r border-[#f2e4d4] bg-[#fffdf9]/95 p-3 max-md:w-full max-md:border-b max-md:border-r-0">
        <div className="mb-3 text-xs font-black uppercase tracking-widest text-[#a5917d]">Mis espacios</div>
        <button type="button" onClick={() => props.createCanvas('Nuevo canvas')} className="mb-3 flex w-full items-center justify-center gap-2 rounded-2xl bg-[#7C3AED] px-3 py-2 text-sm font-black text-white"><Plus size={15} />Nuevo</button>
        <div className="space-y-2">
          {props.canvases.map(canvas => (
            <div key={canvas.id} className={`flex items-center gap-1 rounded-2xl border p-1 ${canvas.id === props.activeId ? 'border-[#ddd6fe] bg-[#f5f3ff]' : 'border-[#f2e4d4] bg-white'}`}>
              <button type="button" onClick={() => props.setActiveId(canvas.id)} className="min-w-0 flex-1 px-2 py-1 text-left">
                ...
                <small className="text-xs text-[#a5917d]">{canvas.updated_at ? new Date(canvas.updated_at).toLocaleDateString('es-ES') : 'Nuevo'}</small>
              </button>
              {props.canvases.length > 1 && <button type="button" onClick={() => props.deleteCanvas(canvas)} className="rounded-xl p-2 text-red-500"><Trash2 size={14} /></button>}
            </div>
          ))}
        </div>
      </aside>

      <div className="absolute left-60 right-4 top-4 z-30 flex flex-wrap items-center gap-2 rounded-2xl border border-[#f2e4d4] bg-white/95 p-2 shadow-xl max-md:left-3">
        <div className="flex items-center gap-2 rounded-full bg-[#f5f3ff] px-3 py-2 text-sm font-black text-[#7C3AED]"><CurrentIcon size={16} />{currentTool.label}</div>
        <input className="min-w-36 flex-1 rounded-xl border border-[#f2e4d4] bg-white px-3 py-2 text-sm font-black text-[#172033]" value={props.canvasName} onChange={event => props.renameCanvas(event.target.value)} />
        ...
        <span className="min-w-14 text-center text-sm font-black text-[#7c6f64]">{Math.round(props.zoom * 100)}%</span>
        ...
        <button type="button" onClick={props.exportPng} className="flex items-center gap-2 rounded-xl bg-[#7C3AED] px-3 py-2 text-sm font-black text-white"><Download size={16} />PNG</button>
        <div className="ml-auto flex items-center gap-1 text-xs font-black text-[#7c6f64]"><Save size={14} />{props.saveStatus}</div>
      </div>

      <div className="absolute left-60 top-24 z-30 grid grid-cols-1 gap-2 rounded-2xl border border-[#f2e4d4] bg-white/95 p-2 shadow-xl max-md:bottom-3 max-md:left-3 max-md:right-3 max-md:top-auto max-md:grid-cols-7">
        {TOOL_ITEMS.map(item => {
          const Icon = item.icon
          return <button key={item.id} type="button" title={item.label} onClick={() => props.setTool(item.id)} className={`flex h-11 w-11 items-center justify-center rounded-2xl ${props.tool === item.id ? 'bg-[#7C3AED] text-white' : 'bg-[#fffaf5] text-[#7c6f64]'}`}><Icon size={18} /></button>
        })}
      </div>

      <div className="absolute left-80 top-24 z-30 flex max-w-[calc(100%-22rem)] flex-wrap items-center gap-2 rounded-2xl border border-[#f2e4d4] bg-white/95 p-2 shadow-xl max-md:left-3 max-md:right-3 max-md:top-28 max-md:max-w-none">
        ...
        <button type="button" onClick={props.selectAll} className="rounded-xl border border-[#f2e4d4] bg-white px-3 py-2 text-xs font-black text-[#7c6f64]">Seleccionar todo</button>
      </div>
    </>
  )
}
...
function ToolbarButton({ children, disabled, onClick }: { children: ReactNode; disabled?: boolean; onClick: () => void }) {
  return <button type="button" disabled={disabled} onClick={onClick} className="flex h-9 w-9 items-center justify-center rounded-xl border border-[#f2e4d4] bg-white text-[#7c6f64] disabled:opacity-35">{children}</button>
}
