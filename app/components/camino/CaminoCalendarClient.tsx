        ...
        <section className="grid gap-5 lg:grid-cols-[1fr_280px]">
          ...
          <div className="grid gap-4 content-start">
            <div className="rounded-[28px] border border-blue-100 bg-white px-4 py-3 shadow-[0_4px_16px_rgba(37,99,235,0.08)]">
              <div className="flex flex-wrap items-center gap-2">
                <span className="rounded-full px-2.5 py-1 text-[11px] font-bold" style={{ background: division.bg, color: division.text }}>{division.name}</span>
                <span className="text-xs font-black text-slate-700">{displayedXP.toLocaleString('es-ES')} XP</span>
              </div>
              {caminoPlanId === 'free' && daysSinceReg !== null && (
                <p className="mt-2 text-[11px] font-semibold text-slate-400">{Math.max(0, 7 - daysSinceReg)} días de prueba</p>
              )}
            </div>
            {showWeeklyGoal ? (
              <div className="rounded-[28px] border border-blue-100 bg-white px-4 py-3 shadow-[0_4px_16px_rgba(37,99,235,0.08)]">
