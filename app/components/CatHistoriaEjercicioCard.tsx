...
export default function CatHistoriaEjercicioCard({ ejercicio, contexto }: { ejercicio: any; contexto: string }) {
  ...
  async function handleImagen(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (failedCount > 0) {
      ...
      console.error('[cat-historia] image_compression_failed', { failedCount })
        ...
        {!contenidoIncompleto && <div className="border-t pt-5" style={{ borderColor: UI.border }}>
          ...
          {modo === 'texto' ? (
            ...
            <div>
              ...
              {imagenes.length > 0 && (
                <div className="mb-3 grid grid-cols-2 gap-3 sm:grid-cols-3">
                  {imagenes.map((img, index) => (
                    <div key={`${img.preview}-${index}`} className="relative overflow-hidden rounded-2xl border bg-white" style={{ borderColor: UI.border }}>
                      <img src={img.preview} alt={`Página ${index + 1}`} loading="lazy" decoding="async" className="h-36 w-full object-cover" />
                      ...
                      <button onClick={() => eliminarImagen(index)} type="button" aria-label={`Quitar página ${index + 1}`} className="absolute right-1.5 top-1.5 flex h-6 w-6 items-center justify-center rounded-full text-white shadow-lg" style={{ backgroundColor: UI.color }}>
                        <X size={12} />
                      </button>
                    </div>
                  ))}
              ...
              <button onClick={() => fileRef.current?.click()} type="button" className="campus-hover flex w-full flex-col items-center justify-center rounded-2xl border-2 border-dashed" style={{ height: imagenes.length > 0 ? 100 : 220, borderColor: UI.accent, backgroundColor: `${UI.light}99`, color: UI.color }}>
                <UploadCloud size={imagenes.length > 0 ? 24 : 34} />
                <span className="mt-2 text-sm font-black">{imagenes.length > 0 ? 'Añadir otra página' : 'Haz clic para subir una foto'}</span>
                {imagenes.length === 0 && <span className="mt-1 text-xs font-semibold" style={{ color: UI.accent }}>Fotografía tu respuesta manuscrita</span>}
              </button>
              {imagenes.length > 1 && (
                <p className="mt-2 text-xs font-semibold text-slate-500">Se corrigen juntas como páginas consecutivas de una misma respuesta.</p>
              )}
