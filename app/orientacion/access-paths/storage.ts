  ...
  try {
    ...
    return {
      ...
      target: {
        ...
        community: typeof target.community === 'string' ? target.community : null,
...
export function createCaminoOrientationContext(pathId: AccessPathId, target: OrientationTarget, estimatedScore: number | null, gap: number | null, subjects: AdmissionSubject[], scenario: AccessScenario, calculationComplete: boolean, now = new Date()): CaminoOrientationContext {
  ...
  return {
    ...
    calculationComplete,
    target: { degreeId: target.degreeId, universityId: target.universityId, degree: target.degree, university: target.university, community: target.community, universityAcronym: target.universityAcronym, referenceScore: target.referenceScore },
    estimatedScore,
    gap,
    impactSubjects,
