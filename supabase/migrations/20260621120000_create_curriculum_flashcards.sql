-- Migration: create curriculum_flashcards table and seed 60 Matemáticas II flashcards
-- Curriculum Spine · Matemáticas II · 12 capítulos · 60 tarjetas
-- Contenido editorial convertido de LaTeX-documento a MathMarkdown/KaTeX.
--
-- Columnas de ordenación:
--   order_label  text  — etiqueta visible del documento ("1","6b","14c"…)
--   sort_order   int   — orden entero estable 1-60 para ORDER BY

create table if not exists public.curriculum_flashcards (
  id                  uuid        primary key default gen_random_uuid(),
  subject             text        not null,
  region              text        not null default 'ambas'
                      check (region in ('madrid','andalucia','ambas')),
...
insert into public.curriculum_flashcards
  (subject, region, chapter_number, chapter_title, block_key,
   order_label, sort_order, title,
   concept_latex, alert_title, alert_latex, worked_case_title, worked_case_latex)
values
  ('mates', 'ambas', 1, 'Matrices', 'Álgebra',
   '1', 1, 'Dimensión de una Matriz',
   'Indica el tamaño de la tabla mediante el número de filas ($m$) y columnas ($n$), escrito como $m \times n$.', NULL, NULL, NULL, 'Dada la matriz $A = \begin{pmatrix} 2 & -1 & 4 \\ 0 & 5 & -3 \end{pmatrix}$:
...
- Identificar elementos: $a_{13} = 4$ (fila 1, col 3) y $a_{22} = 5$ (fila 2, col 2).'),
  ('mates', 'ambas', 1, 'Matrices', 'Álgebra',
   ...
   'Solo se pueden sumar o restar matrices que tengan exactamente la **misma dimensión**. La operación se realiza elemento a elemento en la misma posición.', NULL, NULL, NULL, 'Sean $A = \begin{pmatrix} 1 & 3 \\ -2 & 4 \end{pmatrix}$ y $B = \begin{pmatrix} 5 & 0 \\ 2 & -1 \end{pmatrix}$:
...
- **Resta:** $A - B = \begin{pmatrix} 1-5 & 3-0 \\ -2-2 & 4-(-1) \end{pmatrix} = \begin{pmatrix} -4 & 3 \\ -4 & 5 \end{pmatrix}$'),
  ('mates', 'ambas', 1, 'Matrices', 'Álgebra',
   ...
   'Multiplicar un número real $k$ por una matriz consiste en multiplicar **todos y cada uno** de los elementos de la matriz por ese número.', NULL, NULL, NULL, 'Dado el número $k = -2$ y la matriz $A = \begin{pmatrix} 3 & -1 \\ 0 & 4 \end{pmatrix}$:
...
$$'),
  ('mates', 'ambas', 1, 'Matrices', 'Álgebra',
   ...
   'Solo es posible si el **número de columnas de $A$** es igual al **número de filas de $B$**. Se calcula multiplicando las filas de la primera por las columnas de la segunda.', '¡IMPORTANTE!', 'El producto **no es conmutativo**: $A \cdot B \neq B \cdot A$. ¡Respeta siempre el orden!', NULL, 'Multiplicar $A_{2 \times 3} = \begin{pmatrix} 1 & 2 & 0 \\ 3 & -1 & 4 \end{pmatrix}$ por $B_{3 \times 2} = \begin{pmatrix} 2 & 1 \\ 0 & 3 \\ -1 & 5 \end{pmatrix}$ (El resultado será $2 \times 2$):
...
4. **¡Peligro con el producto!** $(A \cdot B)^t = B^t \cdot A^t$ (el orden de las matrices **se invierte**).', NULL, NULL, 'Caso Práctico: Desarrollar Expresiones con Traspuestas', 'Simplifica la expresión $(2A + B^t)^t$:
...
$$'),
  ('mates', 'ambas', 1, 'Matrices', 'Álgebra',
   ...
   'Para hallar potencias elevadas, calcula $A^2$, $A^3$ y busca la regla o patrón numérico que se va repitiendo.', NULL, NULL, NULL, 'Calcula $A^n$ para $A = \begin{pmatrix} 1 & 3 \\ 0 & 1 \end{pmatrix}$:
...
**Patrón detectado:** El elemento superior derecho es $3 \cdot n$. Por tanto: $A^n = \begin{pmatrix} 1 & 3n \\ 0 & 1 \end{pmatrix}$.'),
  ('mates', 'ambas', 1, 'Matrices', 'Álgebra',
   ...
   'El rango es el número de filas independientes. Usamos el método de Gauss haciendo operaciones entre filas para conseguir que los elementos por debajo de la diagonal principal sean **ceros** (escalonar). El rango es el número de filas finales que no sean por completo ceros.', NULL, NULL, NULL, 'Calcular el rango de $A = \begin{pmatrix} 1 & -2 & 3 \\ 2 & -4 & 7 \\ 3 & -6 & 10 \end{pmatrix}$ mediante Gauss:
...
Al escalonar, nos quedan **2 filas no nulas**. Por lo tanto, el **Rango($A$) = 2**.'),
  ('mates', 'ambas', 1, 'Matrices', 'Álgebra',
   ...
   'Para hallar $A^{-1}$, se monta la matriz conjunta con la identidad $(A \mid I)$ y se aplican operaciones de filas hasta lograr que la identidad pase al lado izquierdo: $(I \mid A^{-1})$.', NULL, NULL, NULL, 'Calcular la inversa de $A = \begin{pmatrix} 1 & 2 \\ 3 & 7 \end{pmatrix}$:
...
¡Ya tenemos la identidad a la izquierda! La matriz inversa resultante es: $A^{-1} = \begin{pmatrix} 7 & -2 \\ -3 & 1 \end{pmatrix}$.'),
  ('mates', 'ambas', 1, 'Matrices', 'Álgebra',
   ...
   'Para despejar la incógnita $X$, multiplicamos por la inversa del elemento que le estorbe, manteniendo estrictamente el mismo lado en el que se añade.', NULL, NULL, NULL, 'Resuelve y despeja $X$ en la ecuación $A \cdot X + B = C$:
...
- **Orden $3 \times 3$ (Sarrus):** Se suman los productos de la diagonal principal y sus paralelas, y se restan los productos de la diagonal secundaria y sus paralelas.', NULL, NULL, 'Caso Práctico: Sarrus de Orden 3', 'Calcular el determinante de $A = \begin{pmatrix} 1 & 2 & 3 \\ 0 & 1 & 4 \\ 2 & 0 & -1 \end{pmatrix}$:
...
5. El determinante de la inversa es el inverso del determinante: $|A^{-1}| = \frac{1}{|A|}$.', NULL, NULL, 'Caso Práctico: Aplicación de Propiedades', 'Sabiendo que $A$ es una matriz $3 \times 3$ con $|A| = 5$, calcula sin desarrollar:
...
- Si intercambias dos filas de posición, el determinante **cambia de signo** (se le pone un menos delante).', NULL, NULL, 'Caso Práctico: Determinante por Gauss', 'Calcular el determinante de $A = \begin{pmatrix} 1 & 3 \\ 2 & 7 \end{pmatrix}$ haciendo ceros:
...
$$
*¡Cuidado!* Cada elemento del adjunto lleva un signo $(-1)^{i+j}$ que cambia los signos en forma de tablero de ajedrez: $\begin{pmatrix} + & - & + \\ - & + & - \\ + & - & + \end{pmatrix}$.', NULL, NULL, 'Caso Práctico: Inversa por Adjuntos', 'Calcular la inversa de $A = \begin{pmatrix} 1 & 2 \\ 3 & 8 \end{pmatrix}$:
...
A \cdot X = B
$$
Donde **$A$** es la matriz de coeficientes, **$X$** es el vector de incógnitas $\begin{pmatrix} x \\ y \\ z \end{pmatrix}$ y **$B$** es el vector de términos independientes. La matriz ampliada se denota como **$A^*$** o **$(A|B)$**.', NULL, NULL, 'Caso Práctico: Convertir a Matriz', 'Dado el sistema: $\begin{cases} x + 2y - z = 3 \\ 3x - y = 1 \end{cases}$ su escritura matricial es:
...
- **Rango($A$) $=$ Rango($A^*$) $<$ nº incógnitas** $\longrightarrow$ **Sistema Compatible Indeterminado (SCI):** Infinitas soluciones.', NULL, NULL, 'Caso Práctico: Discusión de un Sistema', 'Discutir el sistema cuya matriz ampliada escalonada es $A^* = \left(\begin{array}{cc|c} 1 & 2 & 5 \\ 0 & 0 & 3 \end{array}\right)$ con incógnitas $x, y$:
...
- Si es **SCI** (Rango < nº incógnitas) $\rightarrow$ Tiene infinitas soluciones además de la $(0,0,0)$.', NULL, NULL, 'Caso Práctico: Sistema Homogéneo', 'Discute el siguiente sistema sin resolverlo: $\begin{cases} x + 2y = 0 \\ 3x + 6y = 0 \end{cases}$
...
$$
Donde $A_x$ consiste en cambiar la columna de las $x$ por la columna de términos independientes ($B$).', NULL, NULL, 'Caso Práctico: Método de Cramer', 'Resolver el sistema: $\begin{cases} x + 2y = 5 \\ 3x + 7y = 17 \end{cases}$
...
**Truco "Multiplicar Cruzado":** Si quieres hacer un cero entre dos filas, multiplica cada fila por el primer número de la otra fila.', NULL, NULL, 'Caso Práctico Paso a Paso', 'Escalonar la matriz $A = \begin{pmatrix} 1 & 2 & -1 \\ 2 & 1 & 3 \\ 3 & 2 & 1 \end{pmatrix}$:
...
4. **Resolver de abajo hacia arriba** una vez limpio y escalonado.', NULL, NULL, 'Caso Práctico Resuelto (Método de Gauss en Sistemas)', 'Resuelve por el método de Gauss:
...
- **Punto medio:** $M = \left(\frac{x_1+x_2}{2}, \frac{y_1+y_2}{2}\right)$', NULL, NULL, 'Caso Práctico: Vector y Punto Medio', 'Dados $A(-1, 4)$ y $B(3, 2)$:
...
- **Módulo:** $||\vec{u}|| = \sqrt{(u_1)^2 + (u_2)^2 + (u_3)^2}$', NULL, NULL, 'Caso Práctico: Módulo de un Vector', 'Calcula el módulo de $\vec{u} = (1, \ -2, \ 2)$: $||\vec{u}|| = \sqrt{1^2 + (-2)^2 + 2^2} = \sqrt{9} = \mathbf{3}$'),
  ('mates', 'ambas', 4, 'Vectores en el Espacio', 'Geometría',
...
- **Vectores paralelos:** $\frac{u_1}{v_1} = \frac{u_2}{v_2} = \frac{u_3}{v_3} = k$', NULL, NULL, 'Caso Práctico: Combinación Lineal', 'Sean $\vec{u} = (2, \ 0, \ -1)$ y $\vec{v} = (1, \ 3, \ 4)$. Calcula $3\vec{u} - 2\vec{v}$:
...
- **Rango del conjunto:** Número máximo de vectores L.I. que contiene.', NULL, NULL, 'Caso Práctico: Estudiar Independencia Lineal', 'Estudia si $\vec{u}=(1,0,2)$, $\vec{v}=(0,1,1)$ y $\vec{w}=(1,1,3)$ son L.I. o L.D.:
...
- **Sistema de Referencia ($R$):** Punto origen $O(0,0,0)$ más una base.', NULL, NULL, 'Caso Práctico: Expresar en la Base Canónica', 'Si $\vec{v} = (5, \ -3, \ 8)$, su expresión analítica en la base canónica es: $\vec{v} = 5\vec{i} - 3\vec{j} + 8\vec{k}$'),
  ('mates', 'ambas', 4, 'Vectores en el Espacio', 'Geometría',
...
- **Perpendicularidad:** $\vec{u} \perp \vec{v}$ si y solo si $\vec{u} \cdot \vec{v} = 0$', NULL, NULL, 'Caso Práctico: Producto Escalar y Perpendicularidad', 'Determina si $\vec{u} = (2, \ 3, \ -1)$ y $\vec{v} = (5, \ -2, \ 4)$ son perpendiculares:
...
- **Área del Triángulo:** $\frac{1}{2} ||\vec{u} \times \vec{v}||$', NULL, NULL, 'Caso Práctico: Calcular Producto Vectorial y Área', 'Halla el área del triángulo determinado por $\vec{u} = (1, 2, 0)$ y $\vec{v} = (0, 3, 1)$:
...
- **Volumen del Tetraedro:** $\frac{1}{6} |[\vec{u}, \vec{v}, \vec{w}]|$', NULL, NULL, 'Caso Práctico: Volumen de un Tetraedro', 'Calcula el volumen del tetraedro con $\vec{u}=(1,1,0)$, $\vec{v}=(0,2,1)$ y $\vec{w}=(2,0,3)$:
...
- **Implícitas:** Intersección de dos planos $\begin{cases} Ax + By + Cz + D = 0 \\ A''x + B''y + C''z + D'' = 0 \end{cases}$', NULL, NULL, 'Caso Práctico: Ecuaciones de la Recta', 'Halla las ecuaciones de la recta por $A(1, -2, 3)$ con dirección $\vec{v}(4, 0, -1)$:
...
$$', NULL, NULL, 'Caso Práctico: Ecuación del Plano', 'Determina la ecuación del plano por $A(2, 0, 1)$ con vector normal $\vec{n}(1, -3, 2)$:
...
- **Entre recta y plano:** Producto escalar $\vec{v} \cdot \vec{n}$. Si es $\neq 0$ son secantes. Si da 0: paralela o contenida.', NULL, NULL, 'Caso Práctico: Recta y Plano', 'Estudia la posición de la recta con $\vec{v}(1, 2, -1)$ y el plano $2x - y + 3z - 1 = 0$:
...
$$', NULL, NULL, 'Caso Práctico: Ángulo Recta-Plano', 'Halla el ángulo entre la recta con $\vec{v}(1, 0, 1)$ y el plano con $\vec{n}(1, 1, 0)$:
...
$$
P'''' = 2M - P
$$', NULL, NULL, 'Caso Práctico: Punto Simétrico respecto a un Plano', 'Halla el simétrico de $P(1, 0, 3)$ respecto al plano $x + y + z - 1 = 0$:
...
- **Entre dos rectas que se cruzan:** $d(r, s) = \frac{|\det(\vec{u}, \vec{v}, \overrightarrow{AB})|}{||\vec{u} \times \vec{v}||}$', NULL, NULL, 'Caso Práctico: Distancia de Punto a Plano', 'Calcula la distancia del origen $O(0,0,0)$ al plano $3x - 4y + 5 = 0$:
...
- **Volumen de un Tetraedro:** $\frac{1}{6} |\det(\vec{u}, \vec{v}, \vec{w})|$', NULL, NULL, 'Caso Práctico: Área de un Triángulo', 'Calcula el área del triángulo con $\vec{u}(2, 0, 0)$ y $\vec{v}(0, 3, 0)$:
...
3. $\text{Área} = \frac{6}{2} = \mathbf{3 \text{ u}^2}$'),
  ('mates', 'ambas', 7, 'Límites y Continuidad', 'Análisis',
   ...
   'El límite de $f(x)$ en $x = c$ es el valor $L$ al que se aproximan las imágenes cuando $x$ se acerca a $c$. No importa lo que ocurra exactamente en $x = c$, sino el comportamiento en sus cercanías.', NULL, NULL, 'Caso Práctico: Idea Intuitiva', 'Observa el comportamiento de $f(x) = x + 2$ cuando $x \to 3$:
...
$$', NULL, NULL, 'Caso Práctico: Límites Laterales en Funciones a Trozos', 'Determina si existe el límite en $x = 1$ de: $f(x) = \begin{cases} 2x & \text{si } x < 1 \\ 4 - x & \text{si } x \ge 1 \end{cases}$
...
- **Potencia:** $\lim [f(x)^{g(x)}] = L^M$ (si $L > 0$)', NULL, NULL, 'Caso Práctico: Aplicación de Propiedades', 'Sabiendo que $\lim_{x \to c} f(x) = 4$ y $\lim_{x \to c} g(x) = 2$, calcula $\lim_{x \to c} \frac{f(x) + 3}{g(x)^2}$:
...
- **En el infinito:** $\lim_{x \to \pm\infty} f(x) = L$ indica una asíntota horizontal.', NULL, NULL, 'Caso Práctico: Límite Infinito en un Punto', 'Calcula $\lim_{x \to 2^+} \frac{1}{x - 2}$:
...
- $1^\infty$ — se aplica la fórmula directa basada en el número $e$.', NULL, NULL, 'Caso Práctico: Indeterminación $1^\infty$', 'Calcula $\lim_{x \to \infty} \left(\frac{x + 3}{x + 1}\right)^x$:
...
- **Inevitable de salto infinito:** Al menos un límite lateral es $\pm\infty$.', NULL, NULL, 'Caso Práctico: Clasificación de una Discontinuidad', 'Estudia la continuidad de $f(x) = \frac{x^2 - 1}{x - 1}$ en $x = 1$:
...
- **Ecuación de la recta tangente:** $y - f(c) = f''(c) \cdot (x - c)$', NULL, NULL, 'Caso Práctico: Recta Tangente', 'Halla la recta tangente a $f(x) = x^2 + 1$ en $x = 2$:
...
$$', NULL, NULL, 'Caso Práctico: Regla de la Cadena', 'Calcula la derivada de $f(x) = \sin(5x^2)$:
...
$$', NULL, NULL, 'Caso Práctico: Teorema de Rolle', 'Verifica si $f(x) = x^2 - 2x$ cumple Rolle en $[0,2]$ y halla $c$:
...
- **Asíntotas:** Vertical ($x=k$), horizontal ($y=L$) u oblicua ($y=mx+n$).', NULL, NULL, 'Caso Práctico: Asíntota Oblicua', 'Halla la asíntota oblicua de $f(x) = \frac{x^2 + 1}{x}$:
...
- **Segunda Derivada ($f''''$):** Si $f''''(x) > 0$ es convexa ($\cup$); si $f''''(x) < 0$ es cóncava ($\cap$). Clasifica los extremos relativos y halla los puntos de inflexión.', NULL, NULL, 'Caso Práctico: Máximos y Mínimos', 'Clasifica los puntos críticos de $f(x) = x^3 - 3x$:
...
\int f(x) \, dx = F(x) + C
$$
- Sigue propiedades de linealidad: permite extraer constantes y separar sumas o restas.', NULL, NULL, 'Caso Práctico: Propiedades de Linealidad', 'Resuelve $\int (3x^2 + 2x) \, dx$:
...
- **Trigonométricas:** $\int \cos x \, dx = \sin x + C$', NULL, NULL, 'Caso Práctico: Integral Inmediata', 'Resuelve $\int \frac{5}{x} \, dx$: extraemos la constante y aplicamos la primitiva logarítmica: $\mathbf{5 \ln|x| + C}$.'),
  ('mates', 'ambas', 10, 'Integrales', 'Análisis',
...
$$
- **Racionales:** Descomponer $\frac{P(x)}{Q(x)}$ en fracciones simples.', NULL, NULL, 'Caso Práctico: Integración por Partes', 'Calcula $\int x \cos x \, dx$:
...
$$
- **Cálculo de Áreas:** Para evitar que áreas por debajo del eje se resten, calcula las raíces en el intervalo e integra por tramos usando valores absolutos.', NULL, NULL, 'Caso Práctico: Área con Barrow', 'Halla el área encerrada por $f(x) = x^2$ entre $x = 0$ y $x = 3$:
...
- **Leyes de Morgan:** $\overline{A \cup B} = \bar{A} \cap \bar{B}$ y $\overline{A \cap B} = \bar{A} \cup \bar{B}$.', NULL, NULL, 'Caso Práctico: Leyes de Morgan', 'Sabiendo que $P(A \cup B) = 0.7$, halla $P(\bar{A} \cap \bar{B})$:
...
$$
La probabilidad es siempre un valor en $[0, 1]$.', NULL, NULL, 'Caso Práctico: Regla de Laplace', 'Calcula la probabilidad de obtener un número par al lanzar un dado de 6 caras:
...
Caso general compatible: $P(A \cup B) = P(A) + P(B) - P(A \cap B)$.', NULL, NULL, 'Caso Práctico: Sucesos Compatibles', 'Con $P(A) = 0.6$, $P(B) = 0.4$ y $P(A \cap B) = 0.2$:
...
- **Tablas de Contingencia:** Tablas cruzadas para organizar conjuntos con dos características.', NULL, NULL, 'Caso Práctico: Tabla de Contingencia', 'En un grupo de 60 mujeres y 40 hombres, 20 mujeres juegan al tenis y hay 45 tenistas en total. Halla $P(\text{Hombre} \cap \text{Tenis})$:
...
$$', NULL, NULL, 'Caso Práctico: Teorema de Bayes', 'Enfermedad al $1\%$ de la población. Test positivo en el $95\%$ de enfermos y $2\%$ de falsos positivos. Si alguien da positivo ($+$), ¿cuál es la probabilidad de estar enfermo ($E$)?
...
- **Desviación Típica ($\sigma$):** $\sigma = \sqrt{\sigma^2}$.', NULL, NULL, 'Caso Práctico: Cálculo de Parámetros', 'Variable discreta con valores $1$ y $2$ con probabilidades $0.4$ y $0.6$:
...
$$
Parámetros directos: $\mu = n \cdot p$ y $\sigma = \sqrt{n \cdot p \cdot q}$.', NULL, NULL, 'Caso Práctico: Cálculo Binomial', 'Lanzamos una moneda 4 veces. Calcula $P(\text{exactamente 3 caras})$:
...
$$
La probabilidad de desviarse más de $k$ desviaciones típicas de la media es como máximo $\frac{1}{k^2}$.', NULL, NULL, 'Caso Práctico: Acotación de Chebycheff', 'Variable con $\mu = 50$ y $\sigma = 5$. Acota $P(X \notin (40, 60))$:
...
$$', NULL, NULL, 'Caso Práctico: Probabilidad en Variables Continuas', 'Con $f(x) = 0.5$ en $[0, 2]$, calcula $P(1 \le X \le 2)$:
...
$$
Z = \frac{X - \mu}{\sigma}
$$', NULL, NULL, 'Caso Práctico: Uso de Tablas Normales', 'Sea $X \sim N(10, 2)$, calcula $P(X \le 13)$:
...
$$', NULL, NULL, 'Caso Práctico: Aproximación Binomial → Normal', 'Aproxima $X \sim B(100, 0.2)$ a una distribución normal:
