...
export default function Home() {
  ...
  async function corregir() {
    ...
    if (asignatura === 'historia' && preguntaActiva) {
      ...
      const tipoMap2: Record<string, string> = { tema: 'desarrollo de tema', comentario: 'comentario de texto', definicion: 'definición de conceptos', corta: 'respuesta corta' }
      const tipoLabel = tipoMap2[p.tipo] || p.tipo
      promptTexto = 'Eres un corrector oficial de la EBAU de Madrid especializado en Historia de España.\n' +
        ...
        'PUNTUACION MAXIMA: ' + p.puntuacion + ' puntos\n' +
        'CRITERIOS: ' + p.criterios + '\n' +
        (modo === 'imagen' ? 'La imagen adjunta contiene la respuesta manuscrita.' : 'RESPUESTA DEL ESTUDIANTE: ' + respuesta) + '\n' +
        'Corrige con ## headers: ## Nota (X/' + p.puntuacion + '), ## Que esta bien, ## Que falta o mejorar, ## Respuesta modelo'
    } else {
      promptTexto = 'Eres un corrector oficial de la EBAU de Madrid.\n' +
        'PREGUNTA: ' + preguntaActiva?.enunciado + '\n' +
        'PUNTUACION MAXIMA: ' + preguntaActiva?.puntuacion + ' puntos\n' +
        ...
        'Corrige con ## headers: ## Nota, ## Que esta bien, ## Que falta, ## Respuesta completa'
    }
            ...
            {correccion && (
              <div style={{ background: '#fff', borderRadius: '14px', border: '2px solid ' + cfg.color, overflow: 'hidden' }}>
                <div style={{ padding: '16px 24px', background: cfg.color, display: 'flex', alignItems: 'center', gap: '10px' }}>
                  ...
                  <span style={{ fontWeight: 700, color: '#fff', fontSize: '14px', letterSpacing: '0.04em' }}>CORRECCION DE PAUSIA IA</span>
                </div>
                    ...
                    <div key={i} onClick={() => setItemSeleccionado(item)} style={{ background: '#fff', borderRadius: '14px', border: '1px solid #e2e8f0', padding: '20px', cursor: 'pointer', transition: 'box-shadow 0.15s' }} onMouseEnter={e => (e.currentTarget.style.boxShadow='0 4px 12px rgba(0,0,0,0.08)')} onMouseLeave={e => (e.currentTarget.style.boxShadow='none')}>
                      ...
                      {item.enunciado && (
                        <div style={{ fontSize: '13px', color: '#374151', background: '#f8fafc', borderRadius: '8px', padding: '10px 14px', lineHeight: '1.6' }}>
                          <ReactMarkdown remarkPlugins={[remarkMath]} rehypePlugins={[rehypeKatex]} components={mdComponents}>{item.enunciado.substring(0, 200) + (item.enunciado.length > 200 ? '...' : '')}</ReactMarkdown>
                        </div>
                      )}
                    </div>
                  ))}
