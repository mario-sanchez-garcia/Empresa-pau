...
export function CanvasToolbar(props: CanvasToolbarProps) {
  ...
  return (
    <>
      <aside className="w-56 shrink-0 overflow-hidden border-r border-[#e2e8f0] bg-white p-3 max-md:w-full max-md:border-b max-md:border-r-0">
        <div className="mb-3 text-[9px] font-black uppercase tracking-[.14em] text-slate-400">Mis espacios</div>
        <button type="button" onClick={() => props.createCanvas('Nuevo canvas')} className="mb-3 flex w-full items-center justify-center gap-2 rounded bg-[#0f172a] px-3 py-2 text-sm font-black text-white transition hover:bg-slate-800"><Plus size={15} />Nuevo</button>
        <div className="space-y-1">
          {props.canvases.map(canvas => (
            <div key={canvas.id} className={`flex items-center gap-1 rounded border p-1 transition ${canvas.id === props.activeId ? 'border-[#e2e8f0] bg-slate-100' : 'border-[#e2e8f0] bg-white hover:bg-slate-50'}`}>
              <button type="button" onClick={() => props.setActiveId(canvas.id)} className="min-w-0 flex-1 px-2 py-1 text-left">
                <strong className="block truncate text-sm text-[#0f172a]">{canvas.name}</strong>
                <small className="text-xs text-slate-400">{canvas.updated_at ? new Date(canvas.updated_at).toLocaleDateString('es-ES') : 'Nuevo'}</small>
              </button>
              {props.canvases.length > 1 && <button type="button" onClick={() => props.deleteCanvas(canvas)} className="rounded p-2 text-slate-500 transition hover:bg-slate-100 hover:text-slate-900"><Trash2 size={14} /></button>}
            </div>
          ))}
        </div>
      </aside>

      <div className="absolute left-60 right-4 top-4 z-30 flex flex-wrap items-center gap-2 rounded border border-[#e2e8f0] bg-white p-2 max-md:left-3">
        <div className="flex items-center gap-2 rounded bg-slate-100 px-3 py-2 text-sm font-black text-slate-700"><CurrentIcon size={16} />{currentTool.label}</div>
        <input className="min-w-36 flex-1 rounded border border-[#e2e8f0] bg-white px-3 py-2 text-sm font-black text-[#0f172a] outline-none transition focus:border-slate-400" value={props.canvasName} onChange={event => props.renameCanvas(event.target.value)} />
        ...
        <button type="button" onClick={props.exportPng} className="flex items-center gap-2 rounded bg-[#0f172a] px-3 py-2 text-sm font-black text-white transition hover:bg-slate-800"><Download size={16} />PNG</button>
        <div className="ml-auto flex items-center gap-1 text-xs font-black text-slate-500"><Save size={14} />{props.saveStatus}</div>
      </div>

      <div className="absolute left-60 top-24 z-30 grid grid-cols-1 gap-1 rounded border border-[#e2e8f0] bg-white p-1.5 max-md:bottom-3 max-md:left-3 max-md:right-3 max-md:top-auto max-md:grid-cols-7">
        {TOOL_ITEMS.map(item => {
          const Icon = item.icon
          return <button key={item.id} type="button" title={item.label} onClick={() => props.setTool(item.id)} className={`flex h-10 w-10 items-center justify-center rounded transition ${props.tool === item.id ? 'bg-[#0f172a] text-white' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'}`}><Icon size={17} /></button>
        })}
      </div>

      <div className="absolute left-80 top-24 z-30 flex max-w-[calc(100%-22rem)] flex-wrap items-center gap-2 rounded border border-[#e2e8f0] bg-white p-2 max-md:left-3 max-md:right-3 max-md:top-28 max-md:max-w-none">
        ...
        <button type="button" onClick={props.selectAll} className="rounded border border-[#e2e8f0] bg-white px-3 py-2 text-xs font-black text-slate-600 transition hover:bg-slate-100 hover:text-slate-900">Seleccionar todo</button>
      </div>
    </>
  )
}
...
function ToolbarButton({ children, disabled, onClick }: { children: ReactNode; disabled?: boolean; onClick: () => void }) {
  return <button type="button" disabled={disabled} onClick={onClick} className="flex h-9 w-9 items-center justify-center rounded border border-[#e2e8f0] bg-white text-slate-600 transition hover:bg-slate-100 hover:text-slate-900 disabled:opacity-35">{children}</button>
}
