...
import { GENERAL_CORRECTION_GUIDE, OFFICIAL_EXAM_GUIDES } from './exam-guides'
import OfficialCriterionCard from './OfficialCriterionCard'
import styles from './orientation.module.css'

export default function CorrectionGuide({ databaseCriteria }: { databaseCriteria: OfficialCriterion[] }) {
  const guides = useMemo(() => [GENERAL_CORRECTION_GUIDE, ...OFFICIAL_EXAM_GUIDES], [])
  const [guideId, setGuideId] = useState(GENERAL_CORRECTION_GUIDE.id)
  ...
  return (
    <section className={styles.correctionGuide} aria-labelledby="correction-title">
      <div className={styles.correctionHero}><div><span className={styles.eyebrow}><BookOpenCheck size={14} /> Documentación verificada</span><h2 id="correction-title">Cómo se corrige de verdad</h2><p>Primero el criterio oficial. Después, la traducción práctica de Kairo.</p></div><div className={styles.officialSeal}><ShieldCheck size={20} /><span><b>Fuentes oficiales</b><small>PAU Madrid · 2026</small></span></div></div>

      <label className={styles.subjectSelector}><span>Asignatura</span><span className={styles.nativeSelectWrap}><select value={guideId} onChange={event => setGuideId(event.target.value)}>{guides.map(item => <option key={item.id} value={item.id}>{item.subject}</option>)}</select><ChevronDown size={16} /></span></label>
      ...
      <p className={styles.sourceCaveat}>A fecha de verificación, la colección equivalente del curso 2026-2027 aún no está publicada. Se muestra el último modelo oficial disponible y se etiqueta con su curso real.</p>
    </section>
  )
}
