...
Corrige con ## headers: ## Nota, ## Qué está bien, ## Qué falta, ## Respuesta completa\`
    ...
    const res = await fetch('/api/chat', {
      ...
      body: JSON.stringify({ pregunta: promptTexto, imagen: modo === 'imagen' ? imagen : null, imagenTipo: modo === 'imagen' ? imagenTipo : null })
    })
...
Responde solo como Pausia (no repitas el historial).\`
  ...
  const NAV_ITEMS = [
    { id: 'examenes' as Seccion, label: 'Exámenes', icon: '📋', desc: 'Practica y corrige' },
      ...
      <aside style={{ width: '260px', minHeight: '100vh', background: '#0f172a', display: 'flex', flexDirection: 'column', position: 'sticky', top: 0, flexShrink: 0 }}>
        <div style={{ padding: '24px 20px 20px', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <div style={{ width: '36px', height: '36px', borderRadius: '10px', background: 'linear-gradient(135deg, #3b82f6, #1d4ed8)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, color: '#fff', fontSize: '16px' }}>P</div>
            <div>
              <div style={{ color: '#fff', fontWeight: 700, fontSize: '17px', lineHeight: '1' }}>Pausia</div>
              <div style={{ color: '#64748b', fontSize: '11px', marginTop: '3px' }}>Academia IA · EBAU Madrid</div>
            </div>
          </div>
        </div>

        <nav style={{ padding: '16px 12px', flex: 1 }}>
          <div style={{ color: '#475569', fontSize: '10px', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', padding: '0 8px', marginBottom: '8px' }}>Navegación</div>
          {NAV_ITEMS.map(item => (
            <button key={item.id} onClick={() => setSeccion(item.id)} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 12px', borderRadius: '10px', border: 'none', cursor: 'pointer', marginBottom: '4px', textAlign: 'left', background: seccion === item.id ? 'rgba(59,130,246,0.15)' : 'transparent' }}>
              ...
              <div>
                <div style={{ color: seccion === item.id ? '#93c5fd' : '#cbd5e1', fontWeight: seccion === item.id ? 600 : 400, fontSize: '14px', lineHeight: '1' }}>{item.label}</div>
        ...
        <div style={{ padding: '16px', borderTop: '1px solid rgba(255,255,255,0.08)' }}>
          ...
          <button onClick={cerrarSesion} style={{ width: '100%', padding: '8px', borderRadius: '8px', background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.2)', color: '#fca5a5', fontSize: '12px', cursor: 'pointer', fontWeight: 500 }}>Cerrar sesión</button>
        </div>
      </aside>
        ...
        <header style={{ background: '#fff', borderBottom: '1px solid #e2e8f0', padding: '0 32px', height: '64px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 10 }}>
          <div>
            ...
            <div style={{ fontSize: '12px', color: '#94a3b8', marginTop: '2px' }}>
              {seccion === 'examenes' && 'Practica con exámenes oficiales de la EBAU Madrid'}
              {seccion === 'chat' && 'Resuelve cualquier duda con tu tutor IA'}
              {seccion === 'planning' && 'Organiza tu preparación'}
            </div>
          </div>
          {seccion === 'examenes' && (
        ...
        {seccion === 'examenes' && (
          <main style={{ flex: 1, padding: '28px 32px', maxWidth: '900px', width: '100%', margin: '0 auto' }}>
            <div style={{ background: '#fff', borderRadius: '14px', border: '1px solid #e2e8f0', padding: '20px', marginBottom: '20px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <span style={{ fontSize: '13px', color: '#64748b', fontWeight: 500 }}>Opción:</span>
                {([0, 1] as const).map(op => (
              ...
              <div style={{ background: '#fff', borderRadius: '14px', border: '1px solid #e2e8f0', overflow: 'hidden', marginBottom: '20px' }}>
                <div style={{ padding: '16px 24px', background: cfg.light, borderBottom: \`2px solid \${cfg.accent}\`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
                    ...
                    <span style={{ padding: '2px 10px', borderRadius: '20px', background: '#f1f5f9', color: '#374151', fontSize: '11px', border: '1px solid #e2e8f0' }}>Opción {opcion === 0 ? 'A' : 'B'}</span>
                  </div>
        ...
        {seccion === 'chat' && (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', maxWidth: '800px', width: '100%', margin: '0 auto', padding: '0 32px' }}>
            <div style={{ flex: 1, overflowY: 'auto', padding: '24px 0', display: 'flex', flexDirection: 'column', gap: '16px' }}>
              {mensajes.length === 0 && (
                <div style={{ textAlign: 'center', padding: '60px 20px' }}>
                  ...
                  <div style={{ fontSize: '15px', color: '#64748b', maxWidth: '400px', margin: '0 auto', lineHeight: '1.6' }}>Tu tutor IA para la EBAU de Madrid. Pregúntame lo que quieras sobre matemáticas, historia o cualquier duda de selectividad.</div>
                  <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap', marginTop: '24px' }}>
                    {['¿Cómo es la estructura del examen de mates?', '¿Qué temas caen más en historia?', 'Explícame la Segunda República'].map(s => (
              ...
              {cargandoChat && (
                <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
                  ...
                  <div style={{ padding: '12px 16px', borderRadius: '14px', background: '#fff', border: '1px solid #e2e8f0', color: '#64748b', fontSize: '14px' }}>✍️ Pausia está escribiendo...</div>
                </div>
              )}
              <div ref={chatEndRef} />
            </div>
            <div style={{ padding: '16px 0 24px', borderTop: '1px solid #e2e8f0' }}>
              ...
              <p style={{ textAlign: 'center', fontSize: '11px', color: '#cbd5e1', margin: '8px 0 0' }}>Enter para enviar · Shift+Enter para nueva línea</p>
            </div>
          </div>
        )}
        ...
        {seccion === 'planning' && (
          <main style={{ flex: 1, padding: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ textAlign: 'center' }}>
              ...
              <div style={{ fontSize: '14px', color: '#64748b' }}>Próximamente — organiza tu preparación semana a semana</div>
