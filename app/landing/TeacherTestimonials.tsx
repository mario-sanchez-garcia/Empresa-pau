  ...
  return (
    <div className="v4c-block v4c-dark" style={{ background: '#111' }}>
      <div className="v4c-copy">
        <span style={{ fontFamily: monoFont, fontSize: 10, color: 'rgba(255,255,255,.3)', letterSpacing: '.18em', textTransform: 'uppercase', marginBottom: 20, display: 'block' }}>Para el aula</span>
        <h2 style={{ fontFamily: headingFont, fontSize: 'clamp(36px, 4vw, 52px)', letterSpacing: '.01em', color: '#fff', lineHeight: .95, marginBottom: 20 }}>{hasTestimonials ? 'Los profesores opinan.' : 'Pensado para acompañar al profesor.'}</h2>
        <p style={{ fontSize: 14, lineHeight: 1.8, color: 'rgba(255,255,255,.5)', maxWidth: '44ch' }}>{hasTestimonials ? 'Opiniones reales de docentes que han probado Kairo.' : 'Publicaremos opiniones cuando contemos con testimonios reales, consentidos y verificados.'}</p>
      </div>
      <div className="v4c-data">
        {hasTestimonials ? teacherTestimonials.map(testimonial => (
          <figure key={`${testimonial.displayName}-${testimonial.subject}`} style={{ margin: 0, padding: '18px 0', borderBottom: '1px solid rgba(255,255,255,.08)' }}>
            ...
            <figcaption style={{ marginTop: 10, color: 'rgba(255,255,255,.4)', fontFamily: monoFont, fontSize: 10 }}>{testimonial.displayName} · {testimonial.role} de {testimonial.subject}{testimonial.school ? ` · ${testimonial.school}` : ''}</figcaption>
          </figure>
        )) : (
