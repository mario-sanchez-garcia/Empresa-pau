        ...
        {seccion === 'examenes' && (
          <>
           ...
           {!isPhilosophy && <div className="exams-filter-card" style={{ background: 'white', borderTop: '1px solid #e2e8f0', borderBottom: '2px solid #0f172a', padding: '12px 0', marginBottom: 20 }}>
            ...
            {!isCatalunaExam && preguntaActiva && (
             <div className="exams-question-card" key={preguntaActivaKey} style={{ background: 'white', borderRadius: 22, border: '1px solid ' + cfg.soft, overflow: 'hidden', marginBottom: 22, boxShadow: '0 18px 45px rgba(15,23,42,0.06)' }}>
                <div style={{ padding: '20px 24px', borderBottom: '1px solid ' + cfg.soft, display: 'flex', flexWrap: 'wrap', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, background: cfg.light }}>
                  <div style={{ minWidth: 0 }}>
                    <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 6 }}>
                      <span style={{ fontSize: 11, fontWeight: 900, padding: '4px 10px', borderRadius: 999, background: cfg.color, color: '#fff', whiteSpace: 'nowrap' }}>{examSystemLabel(ccaa)} {examenActivo?.año} · {tipo}</span>
                      {asignatura === 'historia' && diaHistoriaSeleccionado && (
                        <span style={{ fontSize: 11, fontWeight: 900, padding: '4px 10px', borderRadius: 999, border: '1px solid ' + cfg.accent, color: cfg.color }}>{diaHistoriaSeleccionado}</span>
                      )}
                      {(asignatura === 'lengua' || asignatura === 'ingles' || asignatura === 'biologia') && versionExamenSeleccionada && (
                        <span style={{ fontSize: 11, fontWeight: 900, padding: '4px 10px', borderRadius: 999, border: '1px solid ' + cfg.accent, color: cfg.color }}>{versionExamenSeleccionada}</span>
                      )}
                      <span style={{ fontSize: 11, fontWeight: 700, padding: '4px 10px', borderRadius: 999, border: '1px solid ' + cfg.accent, color: cfg.color }}>
                        {asignatura === 'lengua' ? 'Versión' : asignatura === 'ingles' || asignatura === 'biologia' ? 'Sesión' : 'Opción'} {opcionMostrada}
                      </span>
                    </div>
                    {bloqueActivoLabel && <h2 style={{ marginTop: 8, fontSize: 20, fontWeight: 900, color: '#0f172a', lineHeight: 1.2 }}>{bloqueActivoLabel}</h2>}
                  </div>
                  <div style={{ fontSize: 22, fontWeight: 900, color: cfg.color, flexShrink: 0 }}>{formatPts(puntuacionPreguntaActiva)} <span style={{ fontSize: 13, fontWeight: 700 }}>pts</span></div>
                </div>
